...
class InventoryStore {
	...
	validateItem(item) {
		...
		if (!item.assignedTo) {
			addError(
				'assignedTo',
				'Please select the person this is assigned to'
			)
		}
		...
		switch (item.type) {
			...
			case 'computer':
				if (item.year > new Date().getFullYear()) {
					addError(
						'name',
						'Please select a year (future years are not valid)'
					)
				}
				...
				if (!item.serialNumber) {
					addError(
						'serialNumber',
						'Please specify a valid serial number'
					)
				}
				break
			...
			case 'furniture':
				...
				if (!item.manufacturer) {
					addError(
						'manufacturer',
						"Please identify the item's manufacturer"
					)
				}
				break
		}
